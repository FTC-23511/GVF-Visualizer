<script lang="ts">
  ...
  $: path = (() => {
    ...
    lines.forEach((line, idx) => {
      ...
      if (line.controlPoints.length > 2) {
        ...
        lineElem = new Two.Path(points);
        lineElem.automatic = false;
      } else if (line.endPoint.heading === 'tangential' || (line.splineClass && line.splineClass.toLowerCase().includes('tangential'))) {
        // Hermite Spline
        const samples = 100;
        let points = [new Two.Anchor(startPixelX, startPixelY, 0, 0, 0, 0, Two.Commands.move)];
        
        // Start heading should be from the startPoint or previous line's endPoint
  ...
  $: {
    ...
    switch (currentLine.endPoint.heading) {
      ...
      case "constant":
        ...
        break;
      case "tangential":
        let nextPointInches: BasePoint;
        if (currentLine.controlPoints.length > 0) {
          nextPointInches = getCurvePoint(
          ...
          const h0 = degreesToRadians((_startPoint as Point).endDeg ?? (_startPoint as Point).degrees ?? 90);
          const h1 = degreesToRadians(currentLine.endPoint.startDeg ?? currentLine.endPoint.degrees ?? 90);
          nextPointInches = getHermitePoint(
                  linePercent + (currentLine.endPoint.reverse ? -0.01 : 0.01),
                  _startPoint, h0, currentLine.endPoint, h1
          );
        }
  ...
  function saveFile() {
    const javaCode = generateJavaCode(startPoint, lines);
    const blob = new Blob([javaCode], { type: "text/x-java-source" });
    const linkObj = document.createElement("a");
    const url = URL.createObjectURL(blob);

    linkObj.href = url;
    linkObj.download = "Trajectory.java";
